                        ...
                        for buy in list(self.buy_positions):
                            try:
                                ...
                                if order_status == 'FILLED':
                                    ...
                                    await self.trades_logger.log(f"Initial buy order {buy['order_id']} canceled")
                                
                                elif order_status == 'EXPIRED':
                                    await self.trades_logger.log(f"Initial buy order {buy['order_id']} expired")

                                elif order_status == 'EXPIRED_IN_MATCH':
                                    await self.trades_logger.log(f"Initial buy order {buy['order_id']} expired in match")

                                    await self.cancel_all_initial_orders()
                                    await self.reset_grid(self.current_price)
                                    
                                elif order_status == 'REJECTED':
                                    await self.trades_logger.log(f"Initial buy order {buy['order_id']} rejected")
                        ...
                        for sell in list(self.sell_positions):
                            try:
                                ...
                                if order_status == 'FILLED':
                                    ...
                                    await self.trades_logger.log(f"Initial sell order {sell['order_id']} canceled")
                                
                                elif order_status == 'EXPIRED':
                                    await self.trades_logger.log(f"Initial sell order {sell['order_id']} expired")

                                elif order_status == 'EXPIRED_IN_MATCH':
                                    await self.trades_logger.log(f"Initial sell order {sell['order_id']} expired in match")
                                    ...
                                    await self.trades_logger.log(f"Initial sell order {sell['order_id']} rejected")
    ...
    async def cancel_all_initial_orders(self):
        ...
        async with self.throttler:
            try:
                ...
                logging.error(f"Error in cancel_all_initial_orders: {str(e)}")
            ...
            try:
                ...
                if 'buy_order' in trade and 'sell_order' in trade and trade['trade_type'] == 'BUY_SELL':
                    ...
                    if order_status == 'FILLED':
                        ...
                        await self.trades_logger.log(f"Initial sell order {sell_order['order_id']} expired in match")
...
def check_account_balance(binance_client, symbol, asset_a_funds, asset_b_funds):
    """Проверяет, достаточно ли баланса для заданных средств."""
    account_info = binance_client.client.get_account()
    balances = {balance['asset']: float(balance['free']) for balance in account_info['balances']}

    # Извлекаем базовый и котируемый активы из символа
    base_asset, quote_asset = symbol[:-4], symbol[-4:]

    # Проверяем, достаточно ли средств для актива A (котируемый актив)
    if balances.get(quote_asset, 0) < asset_a_funds:
        raise ValueError(f"Недостаточно баланса для {quote_asset}. Требуется: {asset_a_funds}, Доступно: {balances.get(quote_asset, 0)}")

    # Проверяем, достаточно ли средств для актива B (базовый актив)
    if balances.get(base_asset, 0) < asset_b_funds:
        raise ValueError(f"Недостаточно баланса для {base_asset}. Требуется: {asset_b_funds}, Доступно: {balances.get(base_asset, 0)}")

    logging.info(f"Достаточный баланс для {quote_asset} и {base_asset}.")
