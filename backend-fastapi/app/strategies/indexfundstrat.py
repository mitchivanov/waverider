...
class IndexFundStrategy(BaseStrategy):
    """
    ...
    def __init__(
        ...
        self.risk_agreement = risk_agreement
        self.upper_risk_limit = upper_risk_limit
        self.lower_risk_limit = lower_risk_limit
        ...
        asyncio.create_task(self._init_internal_ratio())
        
        self.session = None

    async def _init_internal_ratio(self):
        """Асинхронная инициализация internal_ratio"""
        self.internal_ratio = await self._calculate_internal_ratio()
    ...
    async def _calculate_internal_ratio(self):
        ...
        """
        try:
            if self.asset_a_funds == 0 or self.asset_b_funds == 0:
                await self.trades_logger.fatal("Asset a or asset b is 0. Internal ratio cannot be calculated.")
                raise ValueError("Asset funds cannot be zero")
            
            ratio = float(self.asset_b_funds) / float(self.asset_a_funds)
            await self.trades_logger.log(f"Internal ratio calculated: {ratio}")
            return ratio
            
        except Exception as e:
            await self.trades_logger.error(f"Error calculating internal ratio: {e}")
            raise
    ...
    async def calculate_index_fund_order_size(self):
        ...
        # Рассчитываем шаг цены для гридов
        price_range = self.current_price * self.deviation_threshold
        price_step = price_range / self.grids
        
        self.buy_order_sizes = []
        self.sell_order_sizes = []
        
        # Для buy orders (цена растет)
        for i in range(1, self.grids + 1):
            price = self.current_price + (i * price_step)
            # Q = (O * P - D) / (2 * P)
            quantity = (self.asset_b_funds * price - self.asset_a_funds) / (2 * price)
            self.sell_order_sizes.append(abs(quantity))
            
        # Для sell orders (цена падает)
        for i in range(1, self.grids + 1):
            price = self.current_price - (i * price_step)
            # Q = (O * P - D) / (2 * P)
            quantity = (self.asset_b_funds * price - self.asset_a_funds) / (2 * price)
            self.buy_order_sizes.append(abs(quantity))
        
        await self.trades_logger.log(
            f"Price step: {price_step:.2f}\n"
            f"Current price: {self.current_price:.2f}\n"
            f"Asset A (USDT): {self.asset_a_funds:.2f}\n"
            f"Asset B (BTC): {self.asset_b_funds:.8f}\n"
            f"Buy orders: {[f'{size:.8f}' for size in self.buy_order_sizes]}\n"
            f"Sell orders: {[f'{size:.8f}' for size in self.sell_order_sizes]}"
        )
    ...
    async def place_limit_order(self, price, order_type, isInitial, order_size, recvWindow=2000):
        ...
        async with self.throttler:
            try:
                ...
                await self.trades_logger.debug(f"Place Single - Binance API Response: {order}")
                ...
                await self.trades_logger.panic(f"Place Single - Error placing {order_type.upper()} order at ${price}: {str(e)}")
                ...
                for i in range(0, total_orders, batch_size):
                    ...
                    for level_price, order_size in zip(batch_levels, batch_sizes):
                        try:
                            ...
                            await self.trades_logger.debug(f"Place Batch - Binance API Response: {result}")
                            ...
                            failed_orders += 1
                            retry_orders.append((level_price, order_size))
                            await self.trades_logger.error(f"Place Batch - Error placing {order_type.upper()} order at ${level_price}: {str(e)}")
    ...
    async def check_open_trades(self):
        ...
        for trade in list(self.open_trades):
            await self.trades_logger.log(f"f {trade['trade_type']} trade with details: {trade}. Trade id: {trade['id']}")
            try:
                ...
                if 'buy_order' in trade and 'sell_order' in trade and trade['trade_type'] == 'BUY_SELL':
                    ...
                    if order_status == 'FILLED':
                        ...
                        while retry_count < max_retries:
                            try:
                                new_buy_order = await self.place_limit_order(
                                    price=buy_price,
                                    order_type='buy',
                                    isInitial=True,
                                    order_size=quantity,
                                )
                                if new_buy_order and 'orderId' in new_buy_order:
                                    break
                            except Exception as e:
                                retry_count += 1
                ...
                # Check for sell-buy sequence trades        
                    ...
                    if order_status == 'FILLED':
                        ...
                        while retry_count < max_retries:
                            try:
                                new_sell_order = await self.place_limit_order(
                                    price=sell_price,
                                    order_type='sell',
                                    isInitial=True,
                                    order_size=quantity,
                                )
                                if new_sell_order and 'orderId' in new_sell_order:
                                    break
                            except Exception as e:
                                retry_count += 1
                                if retry_count == max_retries:
